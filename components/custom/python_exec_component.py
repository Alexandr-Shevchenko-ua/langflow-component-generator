...
class PythonExecComponent:
    ...
    def build(self, code: str, timeout_s: int = 3) -> dict:
        ...
        # deny common dangerous patterns (case-insensitive)
        banned = [
            r'\bfrom\s+socket\b',
        ...
        try:
            ...
            try:
                completed = subprocess.run([sys.executable, "-I", tmp.name], capture_output=True, text=True, timeout=timeout_s, env=env)
                return {
                ...
                return {"stdout": "", "stderr": f"Timeout after {timeout_s}s", "returncode": -1, "timed_out": True}
        # static import guard via AST
        except:
            try:
                tree = ast.parse(code)
